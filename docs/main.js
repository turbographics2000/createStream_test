...
function rangeCheckProc(p, trackName, propertyName, subPropertyName, val, half, type, prevState) {
    var constraints = {
        [trackName]: {
            [propertyName]: {
    ...
    navigator.mediaDevices.getUserMedia(constraints).then(stream => {
        ...
        if (subPropertyName === 'min') {
            ...
            p.then(_ => {
                return rangeCheck(p, trackName, propertyName, subPropertyName, val, half / 2, type, 'success');
            });
        } else {
            val = val - half;
            console.log('success', val, half);
        ...
        if (subPropertyName === 'min') {
            ...
            return p.then(_ => {
                return rangeCheck(p, trackName, propertyName, subPropertyName, val, half / 2, type, 'fail');
            });
        } else {
            val = val + half;
            console.log('fail', val, half);
...
function rangeCheck() {
    rangeCheckProc(Promise.resolve(), 'video', 'width', 'min', 100000, 100000 / 2, 'int').then(val => console.log(val));
